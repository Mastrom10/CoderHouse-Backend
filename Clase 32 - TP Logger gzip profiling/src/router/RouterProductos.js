...
routerProductos.get('/:id', async (req, res) => {
    ...
    if (isNaN(idConsulta)) {
        logger.error(`El ID ${idConsulta} no es un numero`);
        return res.send({ error: 'El parámetro ingresado no es un número' })
    } else {
        let ProductoEncontrado = await productosDAO.getProductoById(idConsulta);
        ...
        if (ProductoEncontrado && ProductoEncontrado.id) {
            ...
            logger.error(`El ID ${idConsulta} no existe`);
            return res.send({ error: `No se encontro ningun Producto con el id ${idConsulta}` })
        }
    }
});
...
routerProductos.put('/:id', async (req, res) => {
    ...
    idConsulta = parseInt(req.params.id)
    if (isNaN(idConsulta)) {
        logger.error('El parámetro ingresado no es un número')
        return res.send({ error: 'El parámetro ingresado no es un número' })
    }
